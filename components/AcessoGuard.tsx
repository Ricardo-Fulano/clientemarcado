...
interface AcessoContextType { status: string; checkoutUrl: string }
const AcessoContext = createContext<AcessoContextType>({ status: 'ativo', checkoutUrl: '' })
export function useAcesso() { return useContext(AcessoContext) }

export function AvisoAtraso() {
  const { status, checkoutUrl } = useAcesso()
...
export default function AcessoGuard({ children }: { children: React.ReactNode }) {
      ...
      <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at top,rgba(124,58,237,.12),transparent 50%),linear-gradient(180deg,#060C18,#050B16)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
        <div style={{ width: '100%', maxWidth: '480px', background: 'rgba(15,23,42,.95)', border: '1px solid rgba(124,58,237,.30)', borderRadius: '22px', padding: '40px 32px', textAlign: 'center', boxShadow: '0 32px 80px rgba(0,0,0,.5)' }}>
          ...
          <p style={{ fontSize: '14px', color: '#94A3B8', lineHeight: 1.7, marginBottom: '8px' }}>
            {bloqueado ? 'Identificamos uma pendência na sua mensalidade. Regularize o pagamento para voltar a acessar o ClienteMarcado.' : 'Seu acesso ao ClienteMarcado foi encerrado. Para reativar sua conta, entre em contato com o suporte.'}
          </p>
          <p style={{ fontSize: '13px', color: '#475569', lineHeight: 1.6, marginBottom: '32px' }}>
            {bloqueado ? 'Assim que o pagamento for confirmado, seu acesso será liberado.' : 'Podemos ajudá-lo a reativar sua conta.'}
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {SUPORTE_WHATSAPP && (
              <a href={`https://wa.me/${SUPORTE_WHATSAPP}?text=${encodeURIComponent('Olá! Preciso regularizar meu acesso ao ClienteMarcado.')}`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '44px', background: 'rgba(15,23,42,.88)', color: '#CBD5E1', border: '1px solid rgba(148,163,184,.18)', borderRadius: '12px', textDecoration: 'none', fontSize: '13px', fontWeight: 600 }}>
                Falar com suporte
              </a>
            )}
          </div>
  ...
  return (
    <AcessoContext.Provider value={{ status: status || 'ativo', checkoutUrl: CHECKOUT_URL }}>
      {children}
