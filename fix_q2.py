...
old = "supabase.from('agendamentos').select('id,profissional_id,cliente_nome,servico_id,servicos(id,nome,preco),data_hora,status,valor,valor_total').eq('user_id',user.id).order('data_hora',{ascending:false}),"
