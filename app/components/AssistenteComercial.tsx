...
function buscarResposta(texto: string): string | null {
  ...
  for (const item of KEYWORDS) {
    if (item.words.some(w => lower.includes(w))) return RESPOSTAS[item.key]
  }
  return null
}
...
export default function AssistenteComercial({ checkoutUrl = '/cadastro' }: Props) {
  ...
  useEffect(() => {
    if (aberto && inputRef.current) setTimeout(() => inputRef.current?.focus(), 100)
  }, [aberto])
  ...
  function enviarInput() {
    ...
    if (res) { setPerguntaAtiva('custom'); setResposta(res) }
    else { setPerguntaAtiva('nao-encontrado'); setResposta('') }
    setInput('')
  }
