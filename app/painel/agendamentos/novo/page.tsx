...
.sb{width:240px;min-height:100vh;background:radial-gradient(circle at top left,rgba(139,92,246,.14),transparent 32%),linear-gradient(180deg,#120A14,#08060A);border-right:1px solid #2A1A2F;display:flex;flex-direction:column;position:fixed;top:0;left:0;z-index:30}
.sb-logo{padding:22px 18px 16px;border-bottom:1px solid #2A1A2F;display:flex;align-items:center;gap:10px}
.sb-ic{width:30px;height:30px;border-radius:8px;background:${G};display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 0 20px rgba(236,72,153,.28)}
.sb nav{flex:1;padding:10px 8px;overflow-y:auto}
.nl{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;margin-bottom:2px;text-decoration:none;font-size:13px;font-weight:500;color:#B8AAB8;transition:all .15s;border:1px solid transparent}
.nl:hover{background:rgba(236,72,153,.10);border-color:rgba(236,72,153,.24);color:#F8F4F7}
.nl.on{background:${G};color:#fff;font-weight:700;border-color:rgba(255,255,255,.10);box-shadow:0 0 26px rgba(236,72,153,.28)}
.sb-foot{padding:12px 10px;border-top:1px solid #2A1A2F}
.mhdr{display:none;align-items:center;justify-content:space-between;padding:0 16px;height:56px;background:rgba(8,6,10,.96);backdrop-filter:blur(20px);border-bottom:1px solid #2A1A2F;position:sticky;top:0;z-index:20;width:100%;max-width:100%}
...
.inp:focus{border-color:rgba(236,72,153,.35);box-shadow:0 0 0 3px rgba(236,72,153,.14)}
...
.blk-h{font-size:13px;font-weight:700;color:'#EC4899';margin-bottom:14px;display:flex;align-items:center;gap:7px}
.btn-p{background:${G};color:#fff;border:1px solid rgba(255,255,255,.12);border-radius:14px;height:46px;padding:0 20px;font-size:13px;font-weight:700;box-shadow:0 12px 32px rgba(236,72,153,.28);display:inline-flex;align-items:center;gap:6px;white-space:nowrap;transition:all .18s;font-family:inherit;cursor:pointer;text-decoration:none}
...
.btn-s:hover{border-color:rgba(236,72,153,.38);color:#fff}
      ...
      <div className="main">
        ...
        <div className="pg"><div className="bdy">
          <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'24px',flexWrap:'wrap'}}>
            ...
            <div>
              <h1 style={{fontSize:'20px',fontWeight:800,color:'#F8F4F7',letterSpacing:'-0.03em',marginBottom:'3px'}}>Novo agendamento</h1>
              <p style={{fontSize:'12px',color:'#B8AAB8'}}>Preencha os dados para registrar o horário na agenda.</p>
            </div>
          </div>
          {erros.length>0&&(
            <div style={{background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.28)',borderRadius:'12px',padding:'12px 16px',marginBottom:'16px'}}>
              {erros.map((e,i)=><p key={i} style={{fontSize:'13px',color:'#EF4444',marginBottom:i<erros.length-1?'4px':0}}>âš  {e}</p>)}
            </div>
          )}
          <div style={{width:'100%',maxWidth:'760px',margin:'0 auto'}}>
            <div style={{width:'100%'}}>
              ...
              <div className="crd" style={{padding:'22px',marginBottom:'14px'}}>
                <p style={{fontSize:'13px',fontWeight:700,color:'#EC4899',marginBottom:'14px',display:'flex',alignItems:'center',gap:'7px'}}>
                  <span style={{background:'rgba(236,72,153,.18)',borderRadius:'6px',padding:'2px 9px',fontSize:'11px'}}>1</span>Cliente / Paciente
                </p>
                <div style={{position:'relative',marginBottom:'12px'}}>
                  ...
                  {showSug&&sug.length>0&&(
                    <div style={{position:'absolute',top:'calc(100% + 4px)',left:0,right:0,background:'rgba(24,16,27,.98)',border:'1.5px solid rgba(236,72,153,.28)',borderRadius:'12px',zIndex:50,overflow:'hidden',boxShadow:'0 16px 40px rgba(0,0,0,.55)'}}>
                      {sug.map((c:any)=>(
                        <div key={c.id} onMouseDown={()=>selCli(c)}
                          style={{padding:'10px 14px',fontSize:'13px',color:'#B8AAB8',cursor:'pointer',borderBottom:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:'10px'}}
                          onMouseEnter={e=>(e.currentTarget.style.background='rgba(236,72,153,.12)')}
                          ...
                          <div><p style={{fontWeight:600,color:'#F8F4F7',fontSize:'13px'}}>{c.nome}</p>{c.whatsapp&&<p style={{fontSize:'11px',color:'#B8AAB8'}}>{c.whatsapp}</p>}</div>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="crd" style={{padding:'22px',marginBottom:'14px'}}>
                <p style={{fontSize:'13px',fontWeight:700,color:'#EC4899',marginBottom:'14px',display:'flex',alignItems:'center',gap:'7px'}}>
                  <span style={{background:'rgba(236,72,153,.18)',borderRadius:'6px',padding:'2px 9px',fontSize:'11px'}}>2</span>Serviço e profissional
                </p>
                ...
                <div style={{maxWidth:'200px'}}>
                  ...
                  <div style={{position:'relative'}}>
                    <span style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',fontSize:'13px',color:'#B8AAB8',fontWeight:600,pointerEvents:'none'}}>R$</span>
              ...
              <div className="crd" style={{padding:'22px',marginBottom:'14px'}}>
                <p style={{fontSize:'13px',fontWeight:700,color:'#EC4899',marginBottom:'14px',display:'flex',alignItems:'center',gap:'7px'}}>
                  <span style={{background:'rgba(236,72,153,.18)',borderRadius:'6px',padding:'2px 9px',fontSize:'11px'}}>3</span>Data e horário
                </p>
                <div className="fg2" style={{marginBottom:'14px'}}>
                  ...
                  <div><label className="lbl">Horário *</label>{loadHoras?<p style={{fontSize:12,color:'#B8AAB8',marginTop:4}}>Carregando horarios...</p>:<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginTop:4}}>{(['08:00','08:30','09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00']).map((h:string)=>{const ocup=horasOcupadas.includes(h);const sel=hora===h;return <button key={h} type="button" disabled={ocup} onClick={()=>!ocup&&setHora(h)} style={{padding:'10px 4px',borderRadius:10,fontSize:12,fontWeight:600,cursor:ocup?'not-allowed':'pointer',fontFamily:'inherit',minHeight:40,border:'1px solid '+(sel?'rgba(255,255,255,.12)':ocup?'#2A1A2F':'rgba(236,72,153,.25)'),background:sel?'linear-gradient(135deg,#EC4899,#D946EF,#8B5CF6)':ocup?'rgba(24,16,27,.45)':'rgba(24,16,27,.85)',color:sel?'#fff':ocup?'#B8AAB8':'#B8AAB8',opacity:ocup?.55:1}}>{h}{ocup&&<span style={{display:'block',fontSize:9,color:'#EF4444'}}>ocupado</span>}</button>})}</div>}</div>
                </div>
                <label className="lbl">Status inicial</label>
                <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                  {[{v:'pendente',l:'⏳ Pendente',c:'#FACC15',bg:'rgba(250,204,21,.12)',bd:'rgba(250,204,21,.35)'},{v:'confirmado',l:'✓ Confirmado',c:'#22C55E',bg:'rgba(34,197,94,.12)',bd:'rgba(34,197,94,.35)'}].map(s=>(
                    <button key={s.v} onClick={()=>setStatus(s.v)}
                      style={{background:status===s.v?s.bg:'rgba(24,16,27,.75)',border:`1px solid ${status===s.v?s.bd:'#2A1A2F'}`,borderRadius:'10px',height:'40px',padding:'0 16px',fontSize:'13px',fontWeight:600,color:status===s.v?s.c:'#B8AAB8',cursor:'pointer',fontFamily:'inherit',transition:'all .15s'}}>
                      {s.l}
                    </button>
                  ))}
                </div>
              ...
              <div className="crd" style={{padding:'22px'}}>
                <p style={{fontSize:'13px',fontWeight:700,color:'#EC4899',marginBottom:'14px',display:'flex',alignItems:'center',gap:'7px'}}>
                  <span style={{background:'rgba(236,72,153,.18)',borderRadius:'6px',padding:'2px 9px',fontSize:'11px'}}>4</span>Observações
                </p>
            ...
            <div style={{marginBottom:'16px'}} className="mob-save">
              {erros.length>0&&<div style={{background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.25)',borderRadius:10,padding:'10px 14px',marginBottom:'12px'}}>
                {erros.map((e,i)=><p key={i} style={{fontSize:12,color:'#EF4444',fontWeight:500}}>{e}</p>)}
              </div>}
