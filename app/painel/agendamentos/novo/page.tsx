...
html,body{overflow-x:hidden;background:#07111F}
...
.sb-ic{width:30px;height:30px;border-radius:8px;background:${GRAD};display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
.drw{position:fixed;top:0;left:0;bottom:0;width:280px;max-width:85vw;background:#070F1D;z-index:50;transform:translateX(-100%);transition:transform .28s;display:flex;flex-direction:column;border-right:1px solid rgba(255,255,255,.06)}
...
.pg{background:radial-gradient(circle at 20% 0%,rgba(37,99,235,.12),transparent 35%),linear-gradient(180deg,#07111F,#081421);min-height:100vh}
.bdy{max-width:1060px;margin:0 auto;padding:28px 32px 80px;width:100%;box-sizing:border-box}
.inp{width:100%;background:#111827;border:1.5px solid rgba(148,163,184,.18);border-radius:12px;padding:0 14px;height:48px;font-size:14px;color:#F8FAFC;outline:none;font-family:inherit;transition:border-color .2s,box-shadow .2s;display:block}
.inp:focus{border-color:rgba(59,130,246,.65);box-shadow:0 0 0 3px rgba(59,130,246,.12)}
.ta{height:auto!important;padding:12px 14px!important;min-height:110px;resize:vertical;line-height:1.6}
.lbl{font-size:11px;font-weight:700;color:#CBD5E1;text-transform:uppercase;letter-spacing:.08em;display:block;margin-bottom:7px}
.blk{background:#101B2D;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:24px;margin-bottom:16px}
.blk-h{font-size:13px;font-weight:700;color:#3B82F6;margin-bottom:16px;display:flex;align-items:center;gap:7px}
.blk-n{background:rgba(37,99,235,.2);border-radius:6px;padding:2px 9px;font-size:11px;font-weight:800}
.btn-p{background:${GRAD};color:#fff;border:1px solid rgba(255,255,255,.12);border-radius:12px;height:48px;padding:0 22px;font-size:14px;font-weight:700;box-shadow:0 10px 26px rgba(37,99,235,.28);display:inline-flex;align-items:center;gap:6px;white-space:nowrap;transition:all .18s;font-family:inherit;cursor:pointer;text-decoration:none}
.btn-p:hover{transform:translateY(-1px);box-shadow:0 14px 34px rgba(37,99,235,.38)}
.btn-s{background:rgba(15,23,42,.75);color:#CBD5E1;border:1px solid rgba(148,163,184,.18);border-radius:12px;height:44px;padding:0 16px;font-size:13px;font-weight:600;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;transition:all .18s;font-family:inherit;cursor:pointer;text-decoration:none}
...
const SB=[
  {href:'/painel',l:'Início'},{href:'/painel/agendamentos',l:'Agenda',on:true},
  {href:'/painel/clientes',l:'Clientes'},{href:'/painel/orcamentos',l:'Orçamentos'},
  {href:'/painel/financeiro',l:'Cobranças'},{href:'/painel/financeiro',l:'Pagamentos'},
  {href:'/painel/servicos',l:'Serviços'},{href:'/painel/profissionais',l:'Profissionais'},
  {href:'/painel/relatorio',l:'Relatórios'},{href:'/painel/perfil',l:'Configurações'},
]
...
export default function NovoAgendamento(){
  ...
  function selCli(c:any){setCliId(c.id);setCNome(c.nome);setCWpp(c.whatsapp||'');setCEmail(c.email||'');setBusca(c.nome);setShowSug(false)}
  ...
  async function salvar(){
    ...
    const {error}=await supabase.from('agendamentos').insert({
      user_id:user.id,cliente_id:cliId||null,cliente_nome:cNome.trim(),
  ...
  if(loading)return(<div style={{minHeight:'100vh',background:'#07111F',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'system-ui'}}><p style={{color:'#64748B',fontSize:'14px'}}>Carregando...</p></div>)
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#07111F',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',overflowX:'hidden',width:'100%',position:'relative'}}>
      ...
      <div className="main">
        <div className="mhdr">
          ...
          <div style={{width:'34px',height:'34px',borderRadius:'50%',background:GRAD,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:700,color:'#fff'}}>{ini}</div>
        </div>
        ...
        <div className="pg"><div className="bdy">
          ...
          {/* Erros */}
          {erros.length>0&&(
            <div style={{background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.28)',borderRadius:'12px',padding:'12px 16px',marginBottom:'16px'}}>
          ...
          <div className="layout" style={{display:'flex',gap:'18px',alignItems:'flex-start'}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              {/* Bloco 1 — Cliente */}
              <div className="blk">
                <p className="blk-h"><span className="blk-n">1</span> Cliente / Paciente</p>
                <div style={{position:'relative',marginBottom:'14px'}}>
                  <label className="lbl">Nome *</label>
                  <input className="inp" type="text" placeholder="Nome ou busque cliente existente..." value={busca}
                    onChange={e=>{setBusca(e.target.value);setCNome(e.target.value);setShowSug(true);setCliId('')}}
                    onFocus={()=>busca.length>1&&setShowSug(true)}
                    onBlur={()=>setTimeout(()=>setShowSug(false),150)}/>
                  {showSug&&sug.length>0&&(
                    <div style={{position:'absolute',top:'calc(100% + 4px)',left:0,right:0,background:'#0B1628',border:'1.5px solid rgba(37,99,235,.30)',borderRadius:'12px',zIndex:50,overflow:'hidden',boxShadow:'0 16px 40px rgba(0,0,0,.55)'}}>
                      {sug.map((c:any)=>(
                        <div key={c.id} onMouseDown={()=>selCli(c)}
                          ...
                          <div style={{width:'30px',height:'30px',borderRadius:'50%',background:GRAD,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:700,color:'#fff',flexShrink:0}}>{c.nome.charAt(0).toUpperCase()}</div>
              ...
              {/* Bloco 2 — Serviço */}
              <div className="blk">
                <p className="blk-h"><span className="blk-n">2</span> Serviço e profissional</p>
                ...
                <div style={{maxWidth:'200px'}}>
                  ...
                  <input className="inp" type="number" min="0" step="0.01" placeholder="0,00" value={valor} onChange={e=>setValor(e.target.value)}/>
                </div>
              </div>

              {/* Bloco 3 — Data e hora */}
              <div className="blk">
                <p className="blk-h"><span className="blk-n">3</span> Data e horário</p>
                ...
                <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                  {[{v:'pendente',l:'⏳ Pendente',c:'#F59E0B',bg:'rgba(245,158,11,.12)',bd:'rgba(245,158,11,.35)'},{v:'confirmado',l:'✓ Confirmado',c:'#22C55E',bg:'rgba(34,197,94,.12)',bd:'rgba(34,197,94,.35)'}].map(s=>(
              ...
              {/* Bloco 4 — Obs */}
              <div className="blk">
                <p className="blk-h"><span className="blk-n">4</span> Observações</p>
                <textarea className="inp ta" placeholder="Informações adicionais sobre o atendimento..." value={obs} onChange={e=>setObs(e.target.value)}/>
              </div>
            </div>

            {/* Resumo lateral */}
            <div className="aside" style={{width:'300px',flexShrink:0,position:'sticky',top:'24px'}}>
              <div style={{background:'#101B2D',border:'1.5px solid rgba(37,99,235,.25)',borderRadius:'20px',padding:'20px',boxShadow:'0 12px 30px rgba(0,0,0,.28)'}}>
                <p style={{fontSize:'13px',fontWeight:700,color:'#60A5FA',marginBottom:'16px'}}>Resumo</p>
                {[
                  {l:'Cliente',v:cNome||'—'},
                  {l:'Serviço',v:servs.find(s=>s.id===servId)?.nome||'—'},
                  {l:'Profissional',v:profs.find(p=>p.id===profId)?.nome||'Qualquer'},
                  {l:'Data',v:data?new Date(data+'T12:00:00').toLocaleDateString('pt-BR'):'—'},
                  {l:'Horário',v:hora||'—'},
                  {l:'Status',v:status==='confirmado'?'Confirmado':'Pendente'},
                  {l:'Valor',v:valor?`R$ ${parseFloat(valor).toLocaleString('pt-BR',{minimumFractionDigits:2})}`:valor===''?'—':'R$ 0,00'},
                ].map(r=>(
                  <div key={r.l} style={{display:'flex',justifyContent:'space-between',marginBottom:'10px',paddingBottom:'10px',borderBottom:'1px solid rgba(255,255,255,.06)'}}>
                    <span style={{fontSize:'11px',fontWeight:700,color:'#64748B',textTransform:'uppercase',letterSpacing:'.06em'}}>{r.l}</span>
                    <span style={{fontSize:'13px',color:'#F8FAFC',fontWeight:600,textAlign:'right',maxWidth:'160px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.v}</span>
                  </div>
                ))}
                <div style={{display:'flex',flexDirection:'column',gap:'8px',marginTop:'6px'}}>
                  <button onClick={salvar} disabled={salvando} className="btn-p" style={{width:'100%',height:'48px',opacity:salvando?.7:1}}>
