...
const STATUS_COR: Record<string, {bg:string;color:string;border:string}> = {
  ...
  'Aguardando aprovaÃ§Ã£o': {bg:'rgba(245,158,11,.15)',  color:'#FACC15', border:'rgba(245,158,11,.35)'},
...
export default function Orcamentos() {
  ...
  function resetForm(){
    ...
    setTipo('OrÃ§amento');setTipoOutro('');setTipoDescricao('')
  ...
  function abrirEditar(orc:any){
    ...
    const tipoSalvo=orc.tipo||'OrÃ§amento'
    const tiposPadrao=['OrÃ§amento','Atendimento','Tratamento','Ordem de serviÃ§o','Retorno']
  ...
  async function handleSalvar(){
    ...
    if(itensValidos.length===0) erros.push('Adicione pelo menos um serviÃ§o com nome e valor.')
    ...
    setMensagem(editandoId?'OrÃ§amento atualizado!':'OrÃ§amento criado com sucesso!')
  ...
  function enviarWpp(orc:any){
    ...
    let msg=`OlÃ¡, ${orc.cliente_nome}!\n\nSeu ${orc.tipo} â€” Total: R$ ${fmtBRL(orc.total)}\nPago: R$ ${fmtBRL(orc.valor_pago)}\nSaldo: R$ ${fmtBRL(orc.saldo_restante)}`
    if(orc.link_pagamento) msg+=`\n\nLink:\n${orc.link_pagamento}`
    msg+=`\n\nApÃ³s pagar, envie o comprovante. Obrigado!`
  ...
  function gerarPDF(orc:any){
    ...
    <h1>${perfil?.nome_negocio||'NegÃ³cio'}</h1><p>${orc.tipo} Â· ${fmtData(orc.data)}</p>
    <p><strong>${orc.cliente_nome}</strong> Â· ${orc.cliente_whatsapp||''}</p>
    <table><thead><tr><th>ServiÃ§o</th><th>Qtd</th><th>UnitÃ¡rio</th><th>Total</th></tr></thead><tbody>${linhas}
  ...
  function gerarMsgCobranca(){
    ...
    const neg=perfil?.nome_negocio||'nosso negÃ³cio'
    let msg=`OlÃ¡, ${clienteNome||'cliente'}! Aqui Ã© d${(neg[0]&&'aeiouAEIOUÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“Ãš'.includes(neg[0])?'a ':'o ')}${neg}.\n\nSeu ${tipoDoc}: R$ ${fmtBRL(total)}.\nPago: R$ ${fmtBRL(valorPagoLocal)}. Saldo: R$ ${fmtBRL(saldoLocal)}.`
    if(linkPag) msg+=`\n\nPagamento:\n${linkPag}`
    msg+=`\n\nApÃ³s pagar, envie o comprovante. Obrigado!`
    return msg
  }
  ...
  function editarHpPag(idx:number){
    ...
    const formasPadrao=['Dinheiro','Pix','CartÃ£o de dÃ©bito','CartÃ£o de crÃ©dito','TransferÃªncia','Link de pagamento']
  ...
  return (
    <div className="psb-wrapper" style={{display:'flex',minHeight:'100vh',background:'#050B16',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',overflowX:'hidden',width:'100%',maxWidth:'100%',position:'relative'}}>
      ...
      <div className="psb-main" style={{flex:1,minWidth:0,minHeight:'100vh',display:'flex',flexDirection:'column'}}>
        ...
        {/* Mobile Header â€” inside cm-main so it pushes content down */}
          ...
          <div style={{minHeight:'100vh',background:'#07111F'}}>
            ...
            <div className="cm-lista-topo" style={{padding:'28px 32px 0',maxWidth:'1280px',margin:'0 auto'}}>
              ...
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px',flexWrap:'wrap',marginBottom:'20px'}}>
                <div>
                  <h1 style={{fontSize:'24px',fontWeight:800,color:'#fff',letterSpacing:'-0.02em',marginBottom:'4px'}}>OrÃ§amentos</h1>
                  <p style={{fontSize:'14px',color:'#94A3B8'}}>Crie, acompanhe e envie orÃ§amentos em poucos segundos.</p>
                </div>
                <button
                  ...
                  <span style={{fontSize:'18px',lineHeight:1}}>+</span> Novo orÃ§amento
                </button>
              </div>
              ...
              {/* CARDS ATALHOS â€” grid 2x2 mobile, 4 colunas desktop */}
                            {/* KPIs â€” grid 2x2 mobile, 4 colunas desktop */}
              <div className="cm-kpi-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'20px'}}>
                {[
                  {icon:'ðŸ“‚',label:'Em aberto',valor:totalAberto,fmt:'n',cor:'#3B82F6',bg:'rgba(59,130,246,.12)',border:'rgba(59,130,246,.25)'},
                  {icon:'â³',label:'A receber',valor:totalAReceber,fmt:'brl',cor:'#F59E0B',bg:'rgba(245,158,11,.12)',border:'rgba(245,158,11,.25)'},
                  {icon:'âœ…',label:'Recebido no mÃªs',valor:recebidoMes,fmt:'brl',cor:'#22C55E',bg:'rgba(34,197,94,.12)',border:'rgba(34,197,94,.25)'},
                  {icon:'ðŸ”„',label:'Parciais',valor:parciais,fmt:'n',cor:'#A78BFA',bg:'rgba(167,139,250,.12)',border:'rgba(167,139,250,.25)'},
                ].map(m=>(
              ...
              <div className="cm-busca-filtros" style={{display:'flex',gap:'10px',marginBottom:'16px',alignItems:'center'}}>
                <div className="cm-busca-input" style={{position:'relative',flex:1}}>
                  <span style={{position:'absolute',left:'12px',top:'50%',transform:'translateY(-50%)',color:'#64748B',fontSize:'14px'}}>ðŸ”</span>
                  <input type="text" placeholder="Buscar cliente, contato ou serviÃ§o..." value={filtroCliente} onChange={e=>setFiltroCliente(e.target.value)}
            ...
            <div className="cm-lista-body" style={{padding:'0 32px 60px',maxWidth:'1280px',margin:'0 auto'}}>
              ...
              {orcsFiltrados.length===0?(
                <div style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'20px',padding:'48px 24px',textAlign:'center'}}>
                  ...
                  <p style={{fontSize:'18px',fontWeight:700,color:'#fff',marginBottom:'8px'}}>Nenhum orÃ§amento criado ainda</p>
                  <p style={{fontSize:'13px',color:'#94A3B8',marginBottom:'24px',lineHeight:'1.5'}}>Crie seu primeiro orÃ§amento, registre pagamentos e envie pelo WhatsApp.</p>
                ...
                <>
                  ...
                  <div className="cm-tabela-desktop" style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'20px',overflow:'hidden'}}>
                    <div style={{padding:'16px 24px',borderBottom:'1px solid rgba(255,255,255,.08)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <p style={{fontSize:'14px',fontWeight:700,color:'#fff'}}>OrÃ§amentos recentes</p>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'2fr 1.2fr 1fr 1fr 1fr 120px 110px',padding:'10px 24px',borderBottom:'1px solid rgba(255,255,255,.06)'}}>
                      {['Cliente','Tipo / Data','Total','Pago','Saldo','Status','AÃ§Ãµes'].map(h=>(
                        ...
                        <div key={orc.id}
                          ...
                          <div style={{display:'flex',gap:'5px'}}>
                            ...
                            <button onClick={()=>enviarWpp(orc)}
                              style={{background:'rgba(22,163,74,.2)',border:'1px solid rgba(22,163,74,.3)',borderRadius:'6px',padding:'5px 8px',fontSize:'13px',cursor:'pointer',fontFamily:'inherit'}}>ðŸ’¬</button>
                            <button onClick={()=>abrirEditar(orc)}
                              style={{background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.12)',borderRadius:'6px',padding:'5px 8px',fontSize:'13px',cursor:'pointer',fontFamily:'inherit'}}>âœï¸</button>
                            <button onClick={()=>handleExcluir(orc.id)}
                              style={{background:'rgba(220,38,38,.15)',border:'1px solid rgba(220,38,38,.25)',borderRadius:'6px',padding:'5px 8px',fontSize:'13px',cursor:'pointer',fontFamily:'inherit'}}>ðŸ—‘</button>
                          </div>
                        </div>
                      )
                    })}
                        ...
                        <div key={orc.id} style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'14px',padding:'14px',boxSizing:'border-box' as const,width:'100%',minWidth:0,overflow:'hidden'}}>
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'10px'}}>
                            <div>
                              ...
                              <p style={{fontSize:'12px',color:'#64748B'}}>{orc.tipo} Â· {fmtData(orc.data)}</p>
                            </div>
          ...
          <div className="cm-form-pad cm-content-pad" style={{padding:'24px 32px 60px',maxWidth:'1100px',margin:'0 auto'}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px',flexWrap:'wrap',gap:'12px'}}>
              <div>
                ...
                <h1 style={{fontSize:'22px',fontWeight:800,color:'#fff',letterSpacing:'-0.02em',marginBottom:'2px'}}>{editandoId?'Editar orÃ§amento':'Novo orÃ§amento'}</h1>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'6px',background:'rgba(34,197,94,.15)',border:'1px solid rgba(34,197,94,.25)',borderRadius:'8px',padding:'6px 12px'}}>
                <span style={{fontSize:'13px',color:'#16A34A'}}>âœ“</span>
            ...
            <div className="cm-form-grid" style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:'20px',alignItems:'start'}}>
              ...
              <div style={{minWidth:0}}>
                ...
                <div style={{...card,padding:0,overflow:'hidden'}}>
                  <div onClick={()=>setShowDetalhes(!showDetalhes)}
                    ...
                    <span style={{color:'#64748B',fontSize:'18px',transform:showDetalhes?'rotate(180deg)':'none',transition:'transform .2s'}}>âŒ„</span>
                  </div>
                  {showDetalhes&&(
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginTop:'16px'}}>
                        <div>
                          ...
                          <select style={sel} value={tipo} onChange={e=>{setTipo(e.target.value);if(e.target.value!=='__outro__')setTipoOutro('')}}>
                            {['OrÃ§amento','Atendimento','Tratamento','Ordem de serviÃ§o','Retorno'].map(t=><option key={t} value={t}>{t}</option>)}
                            <option value="__outro__">Outro</option>
                          </select>
                          {tipo==='__outro__'&&(
                            <div style={{marginTop:'8px',display:'flex',flexDirection:'column',gap:'6px'}}>
                              <input style={inp} type="text" placeholder="Ex: AvaliaÃ§Ã£o, Laudo..." value={tipoOutro} onChange={e=>setTipoOutro(e.target.value)} />
                              <input style={inp} type="text" placeholder="DescriÃ§Ã£o (opcional)" value={tipoDescricao} onChange={e=>setTipoDescricao(e.target.value)} />
                            </div>
                          )}
                        </div>
                        <div>
                ...
                <div className="cm-card" style={card}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'4px'}}>
                    ...
                    <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC'}}>ServiÃ§os / Procedimentos</p>
                  </div>
                  <p style={{fontSize:'12px',color:'#94A3B8',marginBottom:'16px'}}>Adicione os serviÃ§os, procedimentos, produtos ou itens deste orÃ§amento.</p>

                  {/* Header tabela â€” oculto no mobile */}
                  <div style={{display:'grid',gridTemplateColumns:'3fr 80px 120px 110px 32px',gap:'8px',marginBottom:'8px'}} className="cm-hide-mobile">
                    {['Nome do serviÃ§o','Qtd.','Valor unitÃ¡rio','Total',''].map(h=>(
                  ...
                  {itens.map((item,idx)=>(
                    <div key={idx} style={{marginBottom:'12px',padding:'14px',background:'#F8FAFC',borderRadius:'12px',border:'1px solid #DCE3EA',width:'100%',maxWidth:'100%',boxSizing:'border-box'}}>
                      ...
                      <div style={{marginBottom:'8px'}}>
                        <label style={{fontSize:'12px',fontWeight:600,color:'#667085',display:'block',marginBottom:'6px',whiteSpace:'normal',lineHeight:'1.3'}}>Nome do serviÃ§o *</label>
                      ...
                      <input style={{...inp,fontSize:'13px',color:'#94A3B8',width:'100%'}} type="text"
                        placeholder="ObservaÃ§Ã£o opcional" value={item.obs} onChange={e=>atualizarItem(idx,'obs',e.target.value)} />
                    </div>
                  ))}
                  ...
                  <div style={{marginTop:'16px',background:BG,borderRadius:'10px',padding:'14px 16px',width:'100%',maxWidth:'100%',boxSizing:'border-box'}}>
                    ...
                    {descontoNum>subtotal&&subtotal>0&&<p style={{fontSize:'11px',color:'#F59E0B',marginTop:'4px',textAlign:'right'}}>âš  Desconto maior que o subtotal.</p>}
                  </div>
                </div>

                {/* Resumo mobile â€” sÃ³ aparece no mobile */}
                <div className="cm-resumo-mobile" style={{display:'none',background:'#fff',borderRadius:'14px',padding:'14px 16px',marginBottom:'12px',border:'1px solid #DCE3EA',boxShadow:'0 1px 3px rgba(0,0,0,.05)'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:'13px'}}>
                      ...
                      <span style={{fontWeight:600,color:clienteNome?'#fff':'#475569',maxWidth:'60%',textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{clienteNome||'NÃ£o informado'}</span>
                    </div>
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:'13px'}}>
                ...
                {isOdonto&&(
                  <div className="cm-card" style={card}>
                    <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'12px'}}>ðŸ¦· Odontograma</p>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'10px'}}>
                      <div><label style={lbl}>Procedimento</label><input style={inp} type="text" placeholder="Ex: RestauraÃ§Ã£o" value={procNome} onChange={e=>setProcNome(e.target.value)} /></div>
                    ...
                    {procOdonto.map((p,i)=>(
                      <div key={i} style={{background:BG,border:'1px solid #DCE3EA',borderRadius:'8px',padding:'10px 12px',display:'flex',justifyContent:'space-between',marginBottom:'6px'}}>
                        <span style={{fontSize:'13px',color:'#374151'}}>{p.dente?`Dente ${p.dente} â€” `:''}  {p.nome} Â· {p.status}</span>
                        <div style={{display:'flex',gap:'8px'}}>
                          <span style={{fontSize:'13px',color:'#059669',fontWeight:700}}>R$ {fmtBRL(parseFloat(p.valor||'0'))}</span>
                          <button onClick={()=>setProcOdonto(prev=>prev.filter((_,j)=>j!==i))} style={{background:'none',border:'none',color:'#EF4444',cursor:'pointer',fontSize:'16px'}}>Ã—</button>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                      ...
                      <div>
                        ...
                        <p style={{fontSize:'12px',color:'#64748B',marginTop:'1px'}}>
                          {valorPagoLocal>0?`Pago: R$ ${fmtBRL(valorPagoLocal)} Â· Saldo: R$ ${fmtBRL(saldoLocal)}`:'Entrada, pagamentos parciais e link de cobranÃ§a.'}
                        </p>
                      </div>
                    </div>
                    <span style={{color:'#64748B',fontSize:'18px',transform:showPagSection?'rotate(180deg)':'none',transition:'transform .2s'}}>âŒ„</span>
                  </div>
                  {showPagSection&&(
                    <div style={{padding:'0 24px 20px',borderTop:'1px solid #F1F4F8'}}>
                      ...
                      <div style={{marginBottom:'14px'}}>
                        ...
                        <p style={{fontSize:'11px',color:'#9CA3AF',marginTop:'5px'}}>O ClienteMarcado apenas organiza a cobranÃ§a. O pagamento serÃ¡ feito pelo link informado pelo seu negÃ³cio.</p>
                      </div>
                      ...
                      <div style={{display:'flex',gap:'8px',flexWrap:'wrap',marginBottom:'16px'}}>
                        <button onClick={()=>navigator.clipboard.writeText(gerarMsgCobranca())}
                          ...
                          ðŸ“‹ Copiar mensagem de cobranÃ§a
                        </button>
                      ...
                      {!clienteWpp&&<p style={{fontSize:'11px',color:'#9CA3AF',marginBottom:'12px'}}>Informe o WhatsApp do cliente para enviar a cobranÃ§a.</p>}

                      {/* Obs pagamento */}
                      <div style={{marginBottom:'16px'}}>
                        <label style={lbl}>ObservaÃ§Ãµes de pagamento</label>
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                              ...
                              <div>
                                ...
                                <select style={sel} value={hpForma} onChange={e=>{setHpForma(e.target.value);if(e.target.value!=='Outro')setHpFormaOutro('')}}>
                                  {['Dinheiro','Pix','CartÃ£o de dÃ©bito','CartÃ£o de crÃ©dito','TransferÃªncia','Link de pagamento','Outro'].map(f=><option key={f}>{f}</option>)}
                                </select>
                              ...
                              <div><label style={lbl}>ObservaÃ§Ã£o</label><input style={inp} type="text" placeholder="Ex: entrada, parcela 2..." value={hpObs} onChange={e=>setHpObs(e.target.value)} /></div>
                              <div style={{display:'flex',gap:'8px'}}>
                          ...
                          <div key={i} style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px',marginBottom:'6px'}}>
                            <div>
                              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'2px'}}>
                                ...
                                <span style={{fontSize:'11px',color:'#9CA3AF'}}>Â· {fmtData(p.data)}</span>
                              </div>
                ...
                <div style={{...card,padding:0,overflow:'hidden'}}>
                  <div onClick={()=>setShowObs(!showObs)}
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                      <span style={{fontSize:'16px'}}>ðŸ“</span>
                      <div>
                        <p style={{fontSize:'14px',fontWeight:700,color:'#fff'}}>ObservaÃ§Ãµes</p>
                        <p style={{fontSize:'12px',color:'#64748B',marginTop:'1px'}}>InformaÃ§Ãµes extras para o cliente ou para sua equipe.</p>
                      </div>
                    </div>
                    <span style={{color:'#64748B',fontSize:'18px',transform:showObs?'rotate(180deg)':'none',transition:'transform .2s'}}>âŒ„</span>
                  </div>
                  {showObs&&(
                    <div style={{padding:'0 18px 18px',borderTop:'1px solid rgba(255,255,255,.08)',display:'flex',flexDirection:'column',gap:'10px',marginTop:'16px',width:'100%',boxSizing:'border-box'}}>
                      <div><label style={lbl}>ObservaÃ§Ã£o do cliente</label>
                        <textarea rows={2} style={{...inp,resize:'none' as const}} placeholder="Alergias, preferÃªncias, histÃ³rico..."
                          value={clienteObs} onChange={e=>setClienteObs(e.target.value)} /></div>
                      <div><label style={lbl}>ObservaÃ§Ãµes do orÃ§amento</label>
                        <textarea rows={3} style={{...inp,resize:'none' as const}} placeholder="InformaÃ§Ãµes adicionais..."
                ...
                <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'12px 16px',background:'rgba(59,130,246,.1)',borderRadius:'10px',border:'1px solid rgba(59,130,246,.2)'}}>
                  <span style={{fontSize:'16px'}}>ðŸ’¡</span>
                  <p style={{fontSize:'12px',color:'#93C5FD'}}>Dica: vocÃª pode adicionar serviÃ§os, descontos e pagamentos parciais.</p>
                </div>
              </div>
              ...
              {/* Coluna direita â€” Resumo sticky */}
                ...
                <div style={{background:'rgba(255,255,255,.06)',borderRadius:'16px',padding:'20px',border:'1px solid rgba(255,255,255,.1)',boxShadow:'0 4px 20px rgba(0,0,0,.3)'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'10px',marginBottom:'16px'}}>
                    <div>
                      ...
                      <p style={{fontSize:'14px',fontWeight:600,color:clienteNome?'#fff':'#475569'}}>{clienteNome||'NÃ£o informado'}</p>
                    </div>
                    <div>
                  ...
                  <div style={{marginTop:'16px',display:'flex',alignItems:'center',gap:'8px',padding:'10px',background:'rgba(255,255,255,.04)',borderRadius:'8px',border:'1px solid rgba(255,255,255,.08)'}}>
                    ...
                    <div>
                      <p style={{fontSize:'12px',fontWeight:600,color:'#fff'}}>Seus dados estÃ£o seguros</p>
            ...
            <div className="cm-detalhe-pad" style={{padding:'28px 32px 60px',maxWidth:'900px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'20px',flexWrap:'wrap'}}>
                <button onClick={()=>{setView('lista');setShowPagForm(false)}}
                  style={{background:'none',border:'none',cursor:'pointer',fontSize:'13px',color:'#667085',fontFamily:'inherit'}}>â† Voltar</button>
                <h2 style={{fontSize:'20px',fontWeight:800,color:'#F8FAFC'}}>{orc.tipo} â€” {orc.cliente_nome}</h2>
                  ...
                  <div style={{marginTop:'14px',background:'#F0F9FF',border:'1.5px solid #BAE6FD',borderRadius:'10px',padding:'16px'}}>
                    ...
                    <div style={{marginBottom:'10px'}}><label style={lbl}>Forma</label>
                      <select value={pagForma} onChange={e=>setPagForma(e.target.value)} style={sel}>
                        {['Pix','Dinheiro','CartÃ£o de crÃ©dito','CartÃ£o de dÃ©bito','TransferÃªncia','Link de pagamento','Outro'].map(f=><option key={f}>{f}</option>)}
                      </select></div>
                    <div style={{marginBottom:'12px'}}><label style={lbl}>ObservaÃ§Ã£o</label><input type="text" placeholder="Opcional" value={pagObs} onChange={e=>setPagObs(e.target.value)} style={inp} /></div>
                    <div style={{display:'flex',gap:'8px'}}>
                ...
                <div className="cm-card" style={card}>
                  ...
                  {(orc.servicos||[]).map((s:any,i:number)=>(
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,.08)'}}>
                      <div><p style={{fontSize:'13px',color:'#F8FAFC',fontWeight:600}}>{s.nome}</p><p style={{fontSize:'11px',color:'#9CA3AF'}}>{s.qtd||1}x Â· R$ {fmtBRL(parseFloat(s.unitario||'0'))}</p></div>
              ...
              <div style={card}>
                ...
                :pagamentos.map((p,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,.08)'}}>
                    <div><p style={{fontSize:'13px',color:'#F8FAFC',fontWeight:600}}>{p.forma} Â· {fmtData(p.data)}</p>{p.observacao&&<p style={{fontSize:'11px',color:'#9CA3AF'}}>{p.observacao}</p>}</div>
                    <span style={{fontSize:'14px',fontWeight:700,color:'#16A34A'}}>R$ {fmtBRL(p.valor)}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={card}>
                <p style={{fontSize:'14px',fontWeight:700,color:'#F8FAFC',marginBottom:'10px'}}>ðŸ‘¤ Cliente</p>
                <p style={{fontSize:'14px',fontWeight:600,color:'#F8FAFC',marginBottom:'4px'}}>{orc.cliente_nome}</p>
                {orc.cliente_whatsapp&&<p style={{fontSize:'13px',color:'#667085',marginBottom:'2px'}}>ðŸ“± {aplicarMascaraTel(orc.cliente_whatsapp)}</p>}
                {orc.cliente_email&&<p style={{fontSize:'13px',color:'#667085'}}>âœ‰ï¸ {orc.cliente_email}</p>}
