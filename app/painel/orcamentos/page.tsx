                        ...
                        <div key={orc.id}
                          ...
                          <div style={{display:'flex',gap:'4px',flexWrap:'nowrap' as const,overflow:'hidden'}}>
                            ...
                            <button onClick={()=>gerarPDF(orc)} title="Baixar PDF"
                              style={{background:'rgba(6,182,212,.15)',border:'1px solid rgba(6,182,212,.30)',borderRadius:'6px',padding:'4px 8px',fontSize:'12px',cursor:'pointer',fontFamily:'inherit',color:'#22D3EE',fontWeight:600}}>PDF</button>
                      ...
                      return (
                        <div key={orc.id} style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'14px',padding:'16px',boxSizing:'border-box' as const}}>
                          ...
                          <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                            <button onClick={()=>{setDetalheId(orc.id);carregarPagamentos(orc.id);setView('detalhe')}}
                              style={{flex:2,minWidth:90,background:'rgba(37,99,235,.2)',border:'1px solid rgba(37,99,235,.3)',borderRadius:'8px',padding:'9px',fontSize:'12px',fontWeight:600,color:'#93C5FD',cursor:'pointer',fontFamily:'inherit'}}>
                              Ver
                            </button>
                            <button onClick={()=>gerarPDF(orc)}
                              style={{flex:1,minWidth:60,background:'rgba(6,182,212,.15)',border:'1px solid rgba(6,182,212,.30)',borderRadius:'8px',padding:'9px',fontSize:'12px',fontWeight:600,color:'#22D3EE',cursor:'pointer',fontFamily:'inherit'}}>
                              📥 PDF
                            </button>
                            <button onClick={()=>enviarWpp(orc)}
                              style={{flex:1,minWidth:60,background:'rgba(22,163,74,.2)',border:'1px solid rgba(22,163,74,.3)',borderRadius:'8px',padding:'9px',fontSize:'12px',fontWeight:600,color:'#4ADE80',cursor:'pointer',fontFamily:'inherit'}}>
                              💬
                            </button>
                            <button onClick={()=>abrirEditar(orc)}
                              style={{flex:1,minWidth:60,background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.12)',borderRadius:'8px',padding:'9px',fontSize:'12px',fontWeight:600,color:'#CBD5E1',cursor:'pointer',fontFamily:'inherit'}}>
                              ✏️
                            </button>
                          </div>
                        </div>
