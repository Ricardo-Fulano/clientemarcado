...
const MOBILE_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .cm-mobile-footer { display:none; position:fixed; bottom:0; left:0; right:0; width:100%; max-width:100%; background:#0B172A; border-top:1px solid rgba(255,255,255,.08); padding:10px 16px calc(10px + env(safe-area-inset-bottom,0px)); z-index:25; flex-direction:column; gap:6px; box-shadow:0 -2px 12px rgba(0,0,0,.07); box-sizing:border-box; }
...
export default function Orcamentos() {
  ...
  async function init(){
    ...
    setUserId(user.id)
    const {data:p,error:ep}=await supabase.from('perfis').select('*').eq('user_id',user.id).single()
    if(ep) console.warn('perfis:',ep.message); else setPerfil(p)
    const {data:profs,error:epr}=await supabase.from('profissionais').select('id,nome').eq('user_id',user.id)
    if(!epr) setProfissionais(profs||[])
  ...
  async function handleSalvar(){
    ...
    if(editandoId){
      ...
      if(error){console.error('update:',error);setMensagem('Erro ao salvar: '+error.message);return}
    } else {
      const {error}=await supabase.from('orcamentos').insert(payload)
      if(error){console.error('insert:',error);setMensagem('Erro ao criar orçamento: '+error.message);return}
    }
            ...
            <div className="cm-form-grid" style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:'20px',alignItems:'start'}}>
              ...
              <div style={{minWidth:0}}>
                ...
                <div className="cm-card" style={card}>
                  ...
                  {itens.map((item,idx)=>(
                    <div key={idx} style={{marginBottom:'12px',padding:'14px',background:'rgba(255,255,255,.06)',borderRadius:'12px',border:'1px solid rgba(148,163,184,.18)',width:'100%',maxWidth:'100%',boxSizing:'border-box'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
                        ...
                        {itens.length>1&&(
                          <button onClick={()=>setItens(prev=>prev.filter((_,i)=>i!==idx))}
                            style={{background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.30)',borderRadius:'6px',color:'#F87171',cursor:'pointer',fontSize:'13px',padding:'3px 8px'}}>
                            Remover
                          </button>
                        )}
                      </div>
                  ...
                  {/* Subtotal */}
                  <div style={{marginTop:'16px',background:'rgba(5,11,22,.8)',borderRadius:'10px',padding:'14px 16px',width:'100%',maxWidth:'100%',boxSizing:'border-box'}}>
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:'13px',color:'#667085',marginBottom:'8px',paddingBottom:'8px',borderBottom:'1px solid rgba(255,255,255,.08)'}}>
                      <span>Desconto</span>
                ...
                <div className="cm-resumo-mobile" style={{display:'none',background:'rgba(11,18,32,.97)',borderRadius:'14px',padding:'14px 16px',marginBottom:'12px',border:'1px solid rgba(255,255,255,.08)'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:'13px'}}>
                      ...
                      <span style={{fontSize:'11px',fontWeight:700,padding:'2px 8px',borderRadius:'999px',background:STATUS_COR[status]?.bg||'rgba(59,130,246,.15)',color:STATUS_COR[status]?.color||'#60A5FA',border:`1px solid ${STATUS_COR[status]?.border||'rgba(59,130,246,.35)'}`}}>{status}</span>
                    </div>
                ...
                {isOdonto&&(
                  <div className="cm-card" style={card}>
                      ...
                      <div key={ai} style={{display:'flex',gap:'4px',flexWrap:'wrap',marginBottom:'8px'}}>
                        {arco.slice(0,arco.length/2).map(n=>(
                          <button key={n} onClick={()=>toggleDente(n)}
                            style={{width:'32px',height:'32px',borderRadius:'6px',border:'1.5px solid '+(dentesSelec.includes(n)?'#22D3EE':'rgba(200,200,210,.3)'),background:dentesSelec.includes(n)?'#083344':'#E5E7EB',color:dentesSelec.includes(n)?'#fff':'#667085',fontSize:'10px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
                            {n}
                          </button>
                        ))}
                        <div style={{width:'1px',background:'rgba(255,255,255,.12)',margin:'0 4px'}} />
                    ...
                    <button onClick={adicionarProcOdonto} disabled={!procNome||dentesSelec.length===0}
                      style={{border:'1.5px dashed rgba(34,211,238,.30)',borderRadius:'8px',padding:'10px',background:'rgba(34,211,238,.05)',color:'#22D3EE',fontSize:'13px',fontWeight:600,cursor:'pointer',fontFamily:'inherit',width:'100%',marginBottom:'10px'}}>
                    ...
                    <div style={{padding:'0 24px 20px',borderTop:'1px solid rgba(255,255,255,.08)'}}>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',margin:'16px 0'}}>
                        {[{l:'Total',v:total,c:'#0F172A'},{l:'Pago',v:valorPagoLocal,c:'#16A34A'},{l:'Saldo',v:saldoLocal,c:saldoLocal>0?'#EA580C':'#16A34A'}].map(f=>(
                          <div key={f.l} style={{background:'rgba(5,11,22,.8)',borderRadius:'8px',padding:'10px',border:'1px solid rgba(255,255,255,.08)'}}>
                      ...
                      <div style={{marginBottom:'14px'}}>
                        ...
                        {exigirSinal&&(
                          <div style={{background:'rgba(5,11,22,.8)',borderRadius:'10px',padding:'14px',border:'1px solid rgba(255,255,255,.08)',display:'flex',flexDirection:'column',gap:'10px'}}>
                      ...
                      <div style={{borderTop:'1px solid rgba(255,255,255,.08)',paddingTop:'14px'}}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
                          ...
                          <button onClick={()=>{setShowHpForm(!showHpForm);setEditandoPagIdx(null);setHpValor('');setHpForma('Pix');setHpFormaOutro('');setHpData(new Date().toISOString().split('T')[0]);setHpObs('')}}
                            style={{background:'rgba(37,99,235,.15)',border:'1px solid rgba(37,99,235,.30)',borderRadius:'6px',padding:'5px 12px',fontSize:'12px',fontWeight:600,color:'#60A5FA',cursor:'pointer',fontFamily:'inherit'}}>
                            + Registrar pagamento
                          </button>
                        </div>
                        ...
                        {histPags.map((p,i)=>(
                          <div key={i} style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'8px',padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px',marginBottom:'6px'}}>
                            ...
                            <div style={{display:'flex',gap:'6px'}}>
                              ...
                              <button onClick={()=>excluirHpPag(i)} style={{background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.30)',borderRadius:'6px',padding:'3px 8px',fontSize:'11px',fontWeight:600,color:'#F87171',cursor:'pointer',fontFamily:'inherit'}}>Excluir</button>
                            </div>
                          </div>
                        ))}
                        {histPags.length>0&&(
                ...
                <div style={{background:'rgba(255,255,255,.06)',borderRadius:'16px',padding:'20px',border:'1px solid rgba(255,255,255,.1)',boxShadow:'0 4px 20px rgba(0,0,0,.3)'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'10px',marginBottom:'16px'}}>
                    ...
                    <div>
                      ...
                      <span style={{fontSize:'12px',fontWeight:600,padding:'3px 10px',borderRadius:'999px',background:STATUS_COR[status]?.bg||'rgba(59,130,246,.15)',color:STATUS_COR[status]?.color||'#60A5FA',border:`1px solid ${STATUS_COR[status]?.border||'rgba(59,130,246,.35)'}`}}>{status}</span>
                    </div>
                  </div>
        ...
        {view==='detalhe'&&orcDetalhe&&(()=>{
          ...
          return (
            <div className="cm-detalhe-pad" style={{padding:'28px 32px 60px',maxWidth:'900px'}}>
              ...
              {mensagem&&<div style={{fontSize:'13px',padding:'10px 14px',borderRadius:'8px',marginBottom:'14px',background:'rgba(34,197,94,.12)',border:'1px solid rgba(34,197,94,.28)',color:'#4ADE80'}}>{mensagem}</div>}
              ...
              <div style={card}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',marginBottom:'14px'}}>
                  {[{l:'Total',v:orc.total,c:'#0F172A'},{l:'Pago',v:orc.valor_pago,c:'#16A34A'},{l:'Saldo',v:orc.saldo_restante,c:orc.saldo_restante>0?'#EA580C':'#16A34A'}].map(f=>(
                    <div key={f.l} style={{background:'rgba(5,11,22,.8)',borderRadius:'8px',padding:'12px',border:'1px solid rgba(255,255,255,.08)'}}>
