                  ...
                  <div key={label} style={{marginBottom:ai===0?'6px':'0'}}>
                    ...
                    <div style={{display:'flex',justifyContent:'center',gap:'2px',overflowX:'auto',paddingBottom:'4px'}}>
                      {arr.slice(0,arr.length/2).map(n=>(
                        <button key={n} className={denteCls(n)} onClick={()=>setDentesSelec(prev=>prev.includes(n)?prev.filter(d=>d!==n):[...prev,n])}>
                          {n}
                          {linhas.some(l=>l.dentes?.includes(n))&&!dentesSelec.includes(n)&&<span className="dot"/>}
                        </button>
                      ))}
