...
const SB_LINKS=[
  {h:'/painel',l:'In├¡cio',I:Home},
  {h:'/painel/agendamentos',l:'Agenda',I:Calendar},
  {h:'/painel/clientes',l:'Clientes',I:Users},
  {h:'/painel/orcamentos',l:'Or├ºamentos',I:ClipboardList},
  {h:'/painel/cobrancas',l:'Cobran├ºas',I:Wallet,on:true},
  {h:'/painel/pagamentos',l:'Pagamentos',I:CreditCard},
  {h:'/painel/servicos',l:'Servi├ºos',I:Sparkles},
  {h:'/painel/profissionais',l:'Profissionais',I:User},
  {h:'/painel/relatorio',l:'Relat├│rios',I:BarChart3},
  {h:'/painel/perfil',l:'Configura├º├Áes',I:Settings},
]
...
export default function Cobrancas(){
  ...
  const filtradas=cobrancas.filter(o=>{
    const _st=String(o.status||'').toLowerCase().trim()
  ...
  function enviarWpp(orc:any){
    ...
    if(!tel)return
    const msg=`Ol├í, ${orc.cliente_nome}!\n\nVoc├¬ tem um saldo pendente de ${fBRL(orc.saldo_restante)} referente a ${orc.tipo}.\n\nPor favor, entre em contato para regularizar. Obrigado!`
    ...
    <aside className="sb">
      ...
      <div className="sb-foot">
        <div style={{display:'flex',alignItems:'center',gap:'10px',background:'rgba(15,23,42,.6)',border:'1px solid rgba(148,163,184,.12)',borderRadius:'10px',padding:'10px 12px'}}>
          ...
          <div style={{minWidth:0}}><p style={{fontSize:'12px',fontWeight:600,color:'#F8FAFC',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{nome||'Meu neg├│cio'}</p><p style={{fontSize:'10px',color:'#64748B'}}>Administrador</p></div>
        </div>
      </div>
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#050B16',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',overflowX:'hidden',width:'100%'}}>
      ...
      <div className="main">
        <div className="mob-hdr">
          ...
          <span style={{fontSize:'14px',fontWeight:800,color:'#F8FAFC'}}>Cobran├ºas</span>
        ...
        <div className="pg"><div className="bdy">
          ...
          <div className="topo-r" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'16px',flexWrap:'wrap',marginBottom:'24px'}}>
            <div>
              <h1 style={{fontSize:'22px',fontWeight:800,color:'#F8FAFC',letterSpacing:'-0.04em',marginBottom:'5px'}}>Cobran├ºas</h1>
              <p style={{fontSize:'13px',color:'#64748B'}}>Veja quem ainda precisa pagar e envie cobran├ºas com rapidez.</p>
            </div>
          ...
          <div className="kpi-grid">
            {[
              ...
              {l:'VENCIDAS',sub:'Cobran├ºas em atraso',v:vencidas,c:'#F87171',bg:'rgba(239,68,68,.10)',bd:'rgba(239,68,68,.28)',I:AlertTriangle},
          ...
          <div style={{position:'relative',marginBottom:'12px'}}>
            ...
            <input type="text" placeholder="Buscar cliente, servi├ºo, WhatsApp ou cobran├ºa..." value={busca} onChange={e=>setBusca(e.target.value)}
          ...
          {filtradas.length===0?(
            <div className="crd" style={{padding:'56px 24px',textAlign:'center'}}>
              ...
              <p style={{fontSize:'16px',fontWeight:700,color:'#F8FAFC',marginBottom:'8px'}}>Nenhuma cobran├ºa em aberto</p>
              <p style={{fontSize:'13px',color:'#64748B',lineHeight:1.6,marginBottom:'6px'}}>Quando houver or├ºamento pendente, cobran├ºa vencida ou saldo restante, tudo aparecer├í aqui.</p>
              <p style={{fontSize:'12px',color:'#475569',marginBottom:'20px'}}>Voc├¬ tamb├®m pode gerar cobran├ºas a partir de um or├ºamento.</p>
              <Link href="/painel/orcamentos/novo" className="btn-p" style={{display:'inline-flex'}}>+ Nova cobran├ºa</Link>
            </div>
          ):(
                  ...
                  <div key={orc.id} className="crd" style={{padding:'18px 20px'}}>
                    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px',marginBottom:'14px',flexWrap:'wrap'}}>
                      <div>
                        ...
                        <p style={{fontSize:'12px',color:'#64748B'}}>{orc.tipo} ┬À {fData(orc.data)}{orc.cliente_whatsapp&&` ┬À ${orc.cliente_whatsapp}`}</p>
                      </div>
