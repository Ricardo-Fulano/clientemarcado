...
export default function Servicos(){
  ...
  async function salvar(){
    ...
    if(error){
      ...
      // Tentar sem profissionais_ids se coluna nao existir
                ...
                return(
                  <div key={s.id} className="crd sv-card" style={{background:s.ativo?'radial-gradient(circle at top left,rgba(124,58,237,.08),transparent 38%),linear-gradient(145deg,rgba(15,23,42,.97),rgba(8,20,33,.99))':'linear-gradient(145deg,rgba(10,18,30,.98),rgba(6,14,24,.99))',border:`1.5px solid ${s.ativo?'rgba(148,163,184,.18)':'rgba(148,163,184,.10)'}`}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'8px',marginBottom:'10px',flexWrap:'wrap'}}>
                      ...
                      <span style={{fontSize:'11px',fontWeight:700,padding:'4px 12px',borderRadius:'999px',background:s.ativo?'rgba(34,197,94,.14)':'rgba(148,163,184,.10)',color:s.ativo?'#4ADE80':'#94A3B8',border:`1px solid ${s.ativo?'rgba(34,197,94,.28)':'rgba(148,163,184,.16)'}`,whiteSpace:'nowrap',flexShrink:0}}>{s.ativo?'Ativo':'Inativo'}</span>
                    </div>
                    {/* Valor em destaque */}
                    {s.preco&&s.preco>0&&<p style={{fontSize:'20px',fontWeight:800,color:'#4ADE80',marginBottom:'8px'}}>{fBRL(s.preco)}</p>}
                    {/* Badges info */}
                    ...
                    <div className="sv-acoes">
                      ...
                      <button onClick={()=>toggleAtivo(s)} style={{height:'36px',padding:'0 14px',background:s.ativo?'rgba(245,158,11,.12)':'rgba(34,197,94,.12)',border:`1px solid ${s.ativo?'rgba(245,158,11,.28)':'rgba(34,197,94,.28)'}`,borderRadius:'8px',fontSize:'12px',fontWeight:600,color:s.ativo?'#FBBF24':'#4ADE80',cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:'4px',whiteSpace:'nowrap'}}><Power size={13}/>{s.ativo?'Desativar':'Ativar'}</button>
                      <button className="sv-excluir" onClick={()=>excluir(s.id)} style={{height:'36px',padding:'0 14px',background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.24)',borderRadius:'8px',color:'#F87171',cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:'4px',fontSize:'12px',fontWeight:600}}><Trash2 size={13}/>Excluir</button>
                    </div>
                  </div>
                )
              })}
