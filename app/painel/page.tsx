...
.sb{width:220px;min-height:100vh;background:#0B172A;display:flex;flex-direction:column;position:fixed;top:0;left:0;z-index:30;border-right:1px solid rgba(255,255,255,.06)}
...
.nl{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:8px;margin-bottom:2px;text-decoration:none;font-size:13px;font-weight:400;color:rgba(255,255,255,.5);transition:all .15s;border-left:2px solid transparent}
...
.mob-hdr{display:none;align-items:center;justify-content:space-between;padding:0 16px;height:56px;background:#0B172A;border-bottom:1px solid rgba(255,255,255,.07);position:sticky;top:0;z-index:20;flex-shrink:0;width:100%}
.drawer{position:fixed;top:0;left:0;bottom:0;width:300px;max-width:85vw;background:#0B172A;z-index:50;transform:translateX(-100%);transition:transform .3s ease;display:flex;flex-direction:column;border-right:1px solid rgba(255,255,255,.08)}
...
.pg{background:#07111F;min-height:100vh}
.body{max-width:1280px;margin:0 auto;padding:28px 32px 64px;width:100%}

/* cards atalho */
.atl-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px}
.atl-card{border-radius:18px;padding:20px;cursor:pointer;text-align:left;text-decoration:none;position:relative;transition:box-shadow .2s,border-color .2s;display:block;width:100%}
.atl-card:hover{box-shadow:0 6px 28px rgba(0,0,0,.4)}
.atl-ico{width:48px;height:48px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;margin-bottom:14px}
...
.kpi-ico{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px;margin-bottom:12px}

/* agenda */
.agenda-card{background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.09);border-radius:16px;overflow:hidden;margin-bottom:20px}
.agenda-hdr{padding:16px 24px;border-bottom:1px solid rgba(255,255,255,.07);display:flex;align-items:center;justify-content:space-between}
.ag-row{display:flex;align-items:center;gap:12px;padding:12px 24px;border-bottom:1px solid rgba(255,255,255,.04);transition:background .15s}
.ag-row:last-child{border-bottom:none}
.ag-row:hover{background:rgba(255,255,255,.03)}
.ag-hora{font-size:12px;font-weight:700;color:#93C5FD;background:rgba(37,99,235,.15);border:1px solid rgba(37,99,235,.3);border-radius:8px;padding:4px 10px;flex-shrink:0;min-width:52px;text-align:center}

/* ações */
.ac-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
.ac-card{border-radius:14px;padding:16px;text-align:left;cursor:pointer;font-family:inherit;width:100%;transition:opacity .2s;text-decoration:none;display:block}
.ac-card:hover{opacity:.82}

/* checklist */
.chk{background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.09);border-radius:16px;padding:20px;margin-bottom:20px}
...
@media(max-width:1023px){
  ...
  .atl-grid{grid-template-columns:1fr 1fr!important;gap:10px}
  .kpi-grid{grid-template-columns:1fr 1fr!important;gap:10px}
  .ac-grid{grid-template-columns:1fr 1fr!important;gap:10px}
  .ag-row{padding:12px 16px}
  .agenda-hdr{padding:14px 16px}
  .topo-btns{flex-direction:column!important;width:100%}
  .topo-btns a{width:100%!important;justify-content:center!important}
  .topo-row{flex-direction:column!important;align-items:flex-start!important}
}
@media(max-width:480px){
  .atl-grid{grid-template-columns:1fr 1fr!important}
...
export default function Painel() {
  ...
  async function init(){
    ...
    // próximos = agendamentos futuros além de hoje
    ...
    const fat=(agsMes||[]).reduce((a:number,ag:any)=>
      a+(parseFloat(ag.servicos?.preco||'0')||0),0)
    setRecMes(fat)

    const abertos=(orcs||[]).filter(o=>!['Pago','Finalizado','Cancelado'].includes(o.status))
    setOrcAbertos(abertos.length)
    setTotalReceber(abertos.reduce((a,o)=>a+(o.saldo_restante||0),0))

    // retornos pendentes (status='retorno' na tabela de agendamentos)
    const {data:rets}=await supabase.from('agendamentos')
      .select('id').eq('user_id',user.id).eq('status','retorno')
    setRetornos(rets?.length||0)

    const pends=(agsH||[]).filter((a:any)=>a.status==='pendente')
    setPendentes(pends.length)
  ...
  function copiarLink(){
    const url=(typeof window!=='undefined'?window.location.origin:'')+'/'+checklist.slug
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#07111F',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',overflowX:'hidden',width:'100%'}}>
      ...
      <div className="main">
        ...
        <div className="body">
          ...
          {/* ── TOPO ── */}
          <div className="topo-row" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'16px',flexWrap:'wrap',marginBottom:'24px'}}>
            <div>
              <h1 style={{fontSize:'24px',fontWeight:800,color:'#fff',letterSpacing:'-0.03em',marginBottom:'4px',lineHeight:1.2}}>
                {nome?`Olá, ${nome} 👋`:'Painel ClienteMarcado'}
              </h1>
              <p style={{fontSize:'14px',color:'#64748B',lineHeight:1.5}}>Acompanhe sua agenda, clientes, cobranças e retornos em um só lugar.</p>
            </div>
            <div className="topo-btns" style={{display:'flex',gap:'8px',flexShrink:0}}>
              <Link href="/painel/agendamentos"
                style={{background:'#2563EB',color:'#fff',borderRadius:'10px',padding:'10px 18px',fontSize:'13px',fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:'6px',boxShadow:'0 4px 16px rgba(37,99,235,.4)',whiteSpace:'nowrap'}}>
                📅 Novo agendamento
              </Link>
              <Link href="/painel/orcamentos"
                style={{background:'rgba(255,255,255,.07)',color:'#CBD5E1',border:'1px solid rgba(255,255,255,.12)',borderRadius:'10px',padding:'10px 18px',fontSize:'13px',fontWeight:600,textDecoration:'none',display:'flex',alignItems:'center',gap:'6px',whiteSpace:'nowrap'}}>
                📋 Novo orçamento
              </Link>
            </div>
          </div>
          ...
          {chkOk&&checklist.slug&&(
            <div style={{background:'rgba(22,163,74,.1)',border:'1px solid rgba(22,163,74,.25)',borderRadius:'16px',padding:'16px 20px',marginBottom:'20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'12px',flexWrap:'wrap'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:'12px',flex:1,minWidth:0}}>
                <div style={{width:'8px',height:'8px',borderRadius:'50%',background:'#22C55E',flexShrink:0,marginTop:'6px',boxShadow:'0 0 8px rgba(34,197,94,.6)'}}/>
                <div style={{minWidth:0}}>
                  <p style={{fontSize:'13px',fontWeight:700,color:'#4ADE80',marginBottom:'3px'}}>Sua página pública está ativa</p>
                  <p style={{fontSize:'12px',color:'#64748B',marginBottom:'3px'}}>Compartilhe este link com seus clientes para receber agendamentos e contatos.</p>
                  <p style={{fontSize:'11px',color:'#374151',wordBreak:'break-all'}}>{urlPub}</p>
                </div>
              </div>
              <div style={{display:'flex',gap:'8px',flexShrink:0,flexWrap:'wrap'}}>
                <a href={'/'+checklist.slug} target="_blank" rel="noopener noreferrer"
                  style={{background:'#22C55E',color:'#fff',borderRadius:'8px',padding:'8px 16px',fontSize:'12px',fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:'5px',whiteSpace:'nowrap'}}>
                  🔗 Ver página
                </a>
                <button onClick={copiarLink}
                  style={{background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.12)',borderRadius:'8px',padding:'8px 14px',fontSize:'12px',fontWeight:600,color:copiado?'#4ADE80':'#94A3B8',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap',transition:'color .2s'}}>
          ...
          {/* ── NOTIF PENDENTES ── */}
          {pendentes>0&&(
            <div style={{background:'rgba(37,99,235,.1)',border:'1px solid rgba(37,99,235,.22)',borderRadius:'12px',padding:'12px 18px',marginBottom:'20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'12px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                <div style={{width:'8px',height:'8px',borderRadius:'50%',background:'#2563EB',flexShrink:0}}/>
                <p style={{fontSize:'13px',fontWeight:600,color:'#93C5FD'}}>{pendentes} agendamento{pendentes>1?'s':''} pendente{pendentes>1?'s':''} hoje</p>
              </div>
              <Link href="/painel/agendamentos" style={{fontSize:'12px',fontWeight:700,color:'#3B82F6',textDecoration:'none',whiteSpace:'nowrap'}}>Ver todos →</Link>
            </div>
          )}
          ...
          {!chkOk&&(
            <div className="chk">
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'14px'}}>
                ...
                <span style={{fontSize:'14px',fontWeight:700,color:'#2563EB'}}>{prog}%</span>
              </div>
              ...
              {itensChk.map(it=>(
                <Link key={it.texto} href={it.feito?'#':it.href}
                  ...
                  <div style={{width:'20px',height:'20px',borderRadius:'50%',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,background:it.feito?'#22C55E':'transparent',border:it.feito?'none':'1px solid rgba(255,255,255,.15)',color:it.feito?'#fff':'transparent'}}>
                    {it.feito?'✓':''}
                  </div>
                  <span style={{fontSize:'13px',flex:1,color:it.feito?'#374151':'#CBD5E1',fontWeight:it.feito?400:500,textDecoration:it.feito?'line-through':'none'}}>{it.texto}</span>
          ...
          <div className="agenda-card">
            <div className="agenda-hdr">
              <div>
                ...
                <p style={{fontSize:'12px',color:'#4B5563',textTransform:'capitalize'}}>{fmtDia()}</p>
              </div>
              <Link href="/painel/agendamentos" style={{fontSize:'12px',fontWeight:700,color:'#3B82F6',textDecoration:'none',whiteSpace:'nowrap'}}>Ver agenda completa →</Link>
            </div>
            ...
            {proxHoje.length===0?(
              <div style={{padding:'40px 24px',textAlign:'center'}}>
                ...
                <Link href="/painel/agendamentos"
                  style={{background:'#2563EB',color:'#fff',borderRadius:'8px',padding:'9px 18px',fontSize:'13px',fontWeight:700,textDecoration:'none',display:'inline-flex',alignItems:'center',gap:'6px'}}>
                  + Novo agendamento
                </Link>
              </div>
            ):(
                ...
                {proxHoje.slice(0,6).map((ag:any)=>{
                  ...
                  return(
                    <div key={ag.id} className="ag-row">
                      ...
                      <div style={{width:'32px',height:'32px',borderRadius:'50%',background:av,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:700,color:'#fff',flexShrink:0}}>
          ...
          <div style={{marginBottom:'32px'}}>
            ...
            <div className="ac-grid">
              {[
                {label:'Novo agendamento', sub:'Agende um atendimento',    icon:'📅',href:'/painel/agendamentos',  bg:'rgba(37,99,235,.1)',  bd:'rgba(37,99,235,.22)'},
                {label:'Novo cliente',     sub:'Cadastre um contato',      icon:'👥',href:'/painel/clientes',      bg:'rgba(6,182,212,.1)', bd:'rgba(6,182,212,.22)'},
                {label:'Novo orçamento',   sub:'Crie em segundos',         icon:'📋',href:'/painel/orcamentos',    bg:'rgba(124,58,237,.1)',bd:'rgba(124,58,237,.22)'},
                {label:'Registrar pagamento',sub:'Marque como recebido',   icon:'💳',href:'/painel/financeiro',    bg:'rgba(34,197,94,.1)', bd:'rgba(34,197,94,.22)'},
                {label:'Novo serviço',     sub:'Cadastre um serviço',      icon:'🛎️',href:'/painel/servicos',      bg:'rgba(245,158,11,.1)',bd:'rgba(245,158,11,.22)'},
                {label:'Novo profissional',sub:'Adicione à equipe',        icon:'👤',href:'/painel/profissionais', bg:'rgba(16,185,129,.1)',bd:'rgba(16,185,129,.22)'},
                {label:'Configurar horários',sub:'Ajuste funcionamento',   icon:'⚙️',href:'/painel/perfil',        bg:'rgba(107,114,128,.1)',bd:'rgba(107,114,128,.22)'},
                {label:'Bloqueios de agenda',sub:'Feche dias ou horários', icon:'🚫',href:'/painel/bloqueios',     bg:'rgba(239,68,68,.1)', bd:'rgba(239,68,68,.22)'},
                {label:'Relatórios',       sub:'Veja indicadores',         icon:'📊',href:'/painel/relatorio',     bg:'rgba(6,182,212,.1)', bd:'rgba(6,182,212,.22)'},
              ].map(a=>(
                <Link key={a.label} href={a.href} className="ac-card"
                  style={{background:a.bg,border:`1px solid ${a.bd}`}}>
                  <span style={{fontSize:'20px',display:'block',marginBottom:'10px'}}>{a.icon}</span>
                  <p style={{fontSize:'13px',fontWeight:700,color:'#fff',marginBottom:'3px',lineHeight:1.3}}>{a.label}</p>
          ...
          {/* Rodapé */}
          <div style={{paddingTop:'20px',borderTop:'1px solid rgba(255,255,255,.06)',display:'flex',justifyContent:'flex-end'}}>
            <button onClick={logout}
              style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)',borderRadius:'8px',padding:'8px 16px',fontSize:'13px',fontWeight:600,color:'#4B5563',cursor:'pointer',fontFamily:'inherit'}}>
              Sair da conta
            </button>
          </div>
