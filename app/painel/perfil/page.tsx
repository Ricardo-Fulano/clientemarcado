...
const SB_LINKS=[
  {h:'/painel',l:'Inicio',I:Home},
  {h:'/painel/agendamentos',l:'Agenda',I:Calendar},
  {h:'/painel/clientes',l:'Clientes',I:Users},
  {h:'/painel/orcamentos',l:'Orcamentos',I:ClipboardList},
  {h:'/painel/cobrancas',l:'Cobrancas',I:Wallet},
  {h:'/painel/pagamentos',l:'Pagamentos',I:CreditCard},
  {h:'/painel/servicos',l:'Servicos',I:Sparkles},
  {h:'/painel/profissionais',l:'Profissionais',I:User},
  {h:'/painel/relatorio',l:'Relatorios',I:BarChart3},
  {h:'/painel/perfil',l:'Configuracoes',I:Settings,on:true},
]

const DIAS=['Dom','Seg','Ter','Qua','Qui','Sex','Sab']
const INTERVALOS=['15 min','30 min','45 min','1 hora']
const ANTECEDENCIAS=['Sem restricao','1 hora antes','2 horas antes','4 horas antes','1 dia antes']

const TEMAS=[
  {id:'padrao',nome:'Padrao ClienteMarcado',desc:'Azul + roxo, ideal para qualquer negocio.',p:'#3B82F6',s:'#7C3AED'},
  {id:'beleza',nome:'Beleza Premium',desc:'Rosa + roxo, ideal para estetica, salao e beleza.',p:'#EC4899',s:'#7C3AED'},
  {id:'saude',nome:'Saude Premium',desc:'Verde + ciano, ideal para clinicas e saude.',p:'#22C55E',s:'#22D3EE'},
...
.dia-btn.on{background:${G};color:#fff;border-color:transparent;box-shadow:0 0 14px rgba(124,58,237,.28)}
.tog{width:38px;height:22px;border-radius:999px;border:none;cursor:pointer;position:relative;flex-shrink:0;transition:background .2s}
.tog span{position:absolute;top:3px;width:16px;height:16px;border-radius:50%;background:#fff;transition:left .2s}
...
export default function Perfil(){
  ...
  const [antecedencia,setAntecedencia]=useState('Sem restricao')
  ...
  async function load(){
    ...
    if(p){
      setNome(p.nome_negocio||'');setSlug(p.slug||'');setEnd(p.endereco||'');setWpp(p.whatsapp||'');setInsta(p.instagram||'');setCidade(p.cidade||'');setDesc(p.descricao||'');setCapUrl(p.capa_url||'')
      ...
      if(p.tema_cor)setTema(p.tema_cor)
    }
  }

  async function salvar(){
    if(!nome.trim()||!slug.trim()){setMsg('Nome e link sao obrigatorios.');return}
    setSalvando(true)
    const slugFmt=slug.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'')
    const payload={user_id:userId,nome_negocio:nome.trim(),slug:slugFmt,endereco:end.trim()||null,whatsapp:wpp.replace(/\D/g,'')||null,instagram:insta.trim()||null,cidade:cidade.trim()||null,descricao:desc.trim()||null,capa_url:capUrl||null,dias_ativos:diasAtivos,horarios,intervalo,abertura_geral:abertura,fechamento_geral:fechamento,antecedencia,tema_cor:tema,updated_at:new Date().toISOString()}
    ...
    setMsg('Perfil salvo com sucesso!');setTimeout(()=>setMsg(''),3000)
  }
  ...
  async function removerCapa(){setCapUrl('')}
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#050B16',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',overflowX:'hidden',width:'100%'}}>
      ...
      <div className="main">
        <div className="mob-hdr">
          ...
          <span style={{fontSize:'14px',fontWeight:800,color:'#F8FAFC'}}>Configuracoes</span>
        ...
        <div className="pg"><div className="bdy">
          ...
          {msg&&<div style={{position:'fixed',top:'20px',left:'50%',transform:'translateX(-50%)',background:msg.includes('rro')?'rgba(239,68,68,.16)':'rgba(34,197,94,.16)',border:`1px solid ${msg.includes('rro')?'rgba(239,68,68,.36)':'rgba(34,197,94,.36)'}`,borderRadius:'10px',padding:'10px 20px',zIndex:99,color:msg.includes('rro')?'#F87171':'#4ADE80',fontSize:'13px',fontWeight:700,backdropFilter:'blur(20px)',whiteSpace:'nowrap'}}>{msg}</div>}
          ...
          <div className="topo-r" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px',flexWrap:'wrap',marginBottom:'24px'}}>
            <div>
              <h1 style={{fontSize:'22px',fontWeight:800,color:'#F8FAFC',letterSpacing:'-0.04em',marginBottom:'5px'}}>Perfil do negocio</h1>
              <p style={{fontSize:'13px',color:'#64748B'}}>Configure como seu negocio aparece para os clientes.</p>
            </div>
            <Link href="/painel" prefetch={false} style={{fontSize:'13px',color:'#64748B',textDecoration:'none',display:'flex',alignItems:'center',gap:'4px',flexShrink:0,padding:'8px 12px',background:'rgba(15,23,42,.72)',border:'1px solid rgba(148,163,184,.14)',borderRadius:'8px',transition:'color .15s'}}>&larr; Voltar ao painel</Link>
          </div>
          ...
          {slug&&(
            <div className="crd" style={{border:'1.5px solid rgba(34,211,238,.24)',background:'radial-gradient(circle at top left,rgba(6,182,212,.10),transparent 40%),linear-gradient(145deg,rgba(15,23,42,.97),rgba(8,20,33,.99))'}}>
              ...
              <div className="link-btns" style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                <button onClick={copiarLink} className="btn-p" style={{gap:'6px'}}>{copied?<Check size={14}/>:<Copy size={14}/>}{copied?'Copiado!':'Copiar link'}</button>
                <a href={`https://wa.me/?text=${encodeURIComponent('Agende comigo: '+pubUrl)}`} target="_blank" rel="noreferrer" className="btn-s" style={{background:'rgba(34,197,94,.14)',border:'1px solid rgba(34,197,94,.28)',color:'#4ADE80'}}>WhatsApp</a>
                <a href={pubUrl} target="_blank" rel="noreferrer" className="btn-s"><ExternalLink size={14}/>Ver pagina</a>
              </div>
            </div>
          )}
          ...
          <div className="crd">
            <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'4px'}}>Informacoes do negocio</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'18px'}}>Dados principais que identificam seu negocio.</p>
            <div style={{marginBottom:'14px'}}>
              <label className="lbl">Nome do negocio *</label>
              <input className="inp" type="text" placeholder="Ex: Barbearia da Cabeca" value={nome} onChange={e=>setNome(e.target.value)}/>
            </div>
            <div style={{marginBottom:'14px'}}>
              <label className="lbl">Link personalizado *</label>
              <div style={{display:'flex',alignItems:'center',gap:'0',background:'rgba(15,23,42,.88)',border:'1.5px solid rgba(148,163,184,.18)',borderRadius:'12px',overflow:'hidden',transition:'border-color .2s'}} onFocusCapture={e=>(e.currentTarget.style.borderColor='rgba(124,58,237,.55)')} onBlurCapture={e=>(e.currentTarget.style.borderColor='rgba(148,163,184,.18)')}>
                <span style={{padding:'0 12px',fontSize:'12px',color:'#64748B',whiteSpace:'nowrap',borderRight:'1px solid rgba(148,163,184,.14)',height:'48px',display:'flex',alignItems:'center',background:'rgba(255,255,255,.03)'}}>clientemarcado.vercel.app/</span>
            ...
            <div>
              <label className="lbl">Endereco (opcional)</label>
              <input className="inp" type="text" placeholder="Ex: Rua Principal, 123 - Sao Paulo" value={end} onChange={e=>setEnd(e.target.value)}/>
            </div>
          </div>

          {/* Dados publicos */}
          <div className="crd">
            <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'4px'}}>Dados publicos do negocio</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'18px'}}>Informacoes que aparecem na sua pagina de agendamento.</p>
            <div className="fg2" style={{marginBottom:'14px'}}>
              <div><label className="lbl">WhatsApp do negocio</label><input className="inp" type="tel" placeholder="(11) 99999-9999" value={wpp} onChange={e=>setWpp(e.target.value)}/></div>
          ...
          {/* Funcionamento */}
          <div className="crd">
            <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'4px'}}>Funcionamento do negocio</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'18px'}}>Defina os dias e horarios em que seus clientes podem agendar.</p>
            ...
            {DIAS.map((d,i)=>{
              ...
              return(
                <div key={d} style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'10px',padding:'12px 14px',background:'rgba(255,255,255,.03)',borderRadius:'10px',border:'1px solid rgba(255,255,255,.06)',flexWrap:'wrap'}}>
                  ...
                  <input type="time" value={horarios[i].abertura} onChange={e=>setHor(i,'abertura',e.target.value)} style={{background:'rgba(15,23,42,.88)',border:'1px solid rgba(148,163,184,.18)',borderRadius:'8px',padding:'6px 10px',fontSize:'13px',color:'#F8FAFC',outline:'none',fontFamily:'inherit',cursor:'pointer'}}/>
                  <span style={{fontSize:'12px',color:'#64748B'}}>ate</span>
                  <input type="time" value={horarios[i].fechamento} onChange={e=>setHor(i,'fechamento',e.target.value)} style={{background:'rgba(15,23,42,.88)',border:'1px solid rgba(148,163,184,.18)',borderRadius:'8px',padding:'6px 10px',fontSize:'13px',color:'#F8FAFC',outline:'none',fontFamily:'inherit',cursor:'pointer'}}/>
                </div>
              )
            })}
          </div>
          ...
          {/* Config agenda */}
          <div className="crd">
            <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'4px'}}>Configuracoes da agenda</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'18px'}}>Controle como o agendamento publico funciona.</p>
            <div className="fg2" style={{marginBottom:'14px'}}>
              <div><label className="lbl">Intervalo entre horarios</label><select className="inp" style={{cursor:'pointer'}} value={intervalo} onChange={e=>setIntervalo(e.target.value)}>{INTERVALOS.map(v=><option key={v}>{v}</option>)}</select></div>
              <div><label className="lbl">Antecedencia minima</label><select className="inp" style={{cursor:'pointer'}} value={antecedencia} onChange={e=>setAntecedencia(e.target.value)}>{ANTECEDENCIAS.map(v=><option key={v}>{v}</option>)}</select></div>
            </div>
            <div className="fg2">
          ...
          {/* Aparencia */}
          <div className="crd">
            <p style={{fontSize:'15px',fontWeight:700,color:'#F8FAFC',marginBottom:'4px'}}>Aparencia da pagina publica</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'18px'}}>Personalize a pagina que seus clientes acessam para agendar.</p>

            {/* Imagem de capa */}
            <p style={{fontSize:'13px',fontWeight:600,color:'#CBD5E1',marginBottom:'8px'}}>Imagem de capa</p>
            <p style={{fontSize:'12px',color:'#64748B',marginBottom:'12px'}}>Aparece no topo da sua pagina de agendamento. Use uma imagem horizontal (16:9).</p>
            {capUrl?(
              <div style={{position:'relative',borderRadius:'14px',overflow:'hidden',marginBottom:'16px',border:'1px solid rgba(148,163,184,.14)'}}>
                ...
                <div style={{position:'absolute',top:'10px',right:'10px',display:'flex',gap:'6px'}}>
                  ...
                  <button onClick={removerCapa} style={{background:'rgba(239,68,68,.18)',border:'1px solid rgba(239,68,68,.36)',color:'#F87171',borderRadius:'8px',padding:'6px 12px',fontSize:'12px',fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>Remover</button>
                </div>
              </div>
            ):(
            ...
            <div style={{borderTop:'1px solid rgba(148,163,184,.10)',paddingTop:'18px',marginTop:'4px'}}>
              ...
              <p style={{fontSize:'12px',color:'#64748B',marginBottom:'14px'}}>Escolha uma cor pronta para combinar com o estilo do seu negocio. Afeta apenas a pagina publica.</p>
