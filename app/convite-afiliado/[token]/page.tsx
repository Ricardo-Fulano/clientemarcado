...
export default function AceitarConviteAfiliado() {
  ...
  useEffect(() => {
    async function validar() {
      const res = await fetch(`/api/convite-afiliado/validar?token=${encodeURIComponent(token)}`)
      const data = await res.json()
      setValido(!!data.valido)
      if (data.valido) {
        setNomeParceiro(data.parceiro?.nome || 'um parceiro')
  ...
  async function aceitarComSenha() {
    ...
    setEnviando(true)
    const res = await fetch('/api/convite-afiliado/aceitar', {
      method: 'POST',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#08060A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
      <div style={{ width: '100%', maxWidth: '420px', background: 'linear-gradient(145deg,rgba(24,16,27,.97),rgba(18,10,20,.99))', border: '1.5px solid #2A1A2F', borderRadius: '20px', padding: '32px 28px', textAlign: 'center' }}>
        <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: G, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', fontSize: '18px', fontWeight: 800, color: '#fff' }}>M</div>
        ...
        {carregando ? (
          ...
          <>
            ...
            <p style={{ fontSize: '14px', color: '#B8AAB8', lineHeight: 1.6, marginBottom: '20px' }}>Você agora tem acesso ao painel de afiliado. Já pode acessar com seu e-mail e senha.</p>
          ...
          <>
            ...
            <p style={{ fontSize: '14px', color: '#B8AAB8', lineHeight: 1.6, marginBottom: '18px' }}>Já existe uma conta MiniPage Pro com o e-mail <strong style={{ color: '#F8F4F7' }}>{emailMascarado}</strong>. Faça login normalmente e depois volte nesta página para confirmar o vínculo.</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <button type="button" onClick={aceitarLogado} disabled={enviando} style={{ background: G, color: '#fff', border: 'none', fontWeight: 700, fontSize: '14px', padding: '13px', borderRadius: '12px', cursor: 'pointer', fontFamily: 'inherit', opacity: enviando ? .7 : 1 }}>
                {enviando ? 'Confirmando...' : 'Já fiz login, confirmar vínculo'}
              </button>
            </div>
          </>
        ) : (
          <>
            <p style={{ fontSize: '19px', fontWeight: 800, color: '#F8F4F7', marginBottom: '6px' }}>Você foi convidado(a) como afiliado</p>
            <p style={{ fontSize: '14px', color: '#B8AAB8', lineHeight: 1.6, marginBottom: '6px' }}>Você foi convidado(a) a acessar o painel de afiliado de</p>
            <p style={{ fontSize: '15px', fontWeight: 700, color: '#EC4899', marginBottom: '20px' }}>{nomeParceiro}</p>
            <p style={{ fontSize: '13px', color: '#B8AAB8', marginBottom: '18px' }}>Crie uma senha para acessar com o e-mail <strong style={{ color: '#F8F4F7' }}>{emailMascarado}</strong>:</p>
