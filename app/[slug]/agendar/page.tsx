                ...
                {Array.from({ length: diasNoMes }).map((_, i) => {
                  ...
                  return (
                    <button key={dia} onClick={() => disponivel && setDataSelecionada(dataStr)} disabled={!disponivel}
                      style={{ padding: '8px 4px', borderRadius: '8px', fontSize: '13px', fontWeight: selecionado || ehHoje ? '700' : '400', cursor: disponivel ? 'pointer' : 'default', border: 'none', textAlign: 'center', transition: 'all 0.15s', background: selecionado ? 'var(--accent)' : 'transparent', color: selecionado ? '#fff' : disponivel ? 'var(--text-primary)' : 'var(--text-muted)', opacity: disponivel ? 1 : 0.35, outline: ehHoje && !selecionado ? '1px solid var(--accent-border)' : 'none' }}>
                      {dia}
                    </button>
                  )
                })}
            ...
            {dataSelecionada && (
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
                <p style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-primary)', marginBottom: '14px' }}>
                  Horários disponíveis — {formatarDataExibicao(dataSelecionada)}
                </p>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                          {periodo.horarios.map(h => (
                            <button key={h} onClick={() => setHorarioSelecionado(h)}
                              style={{ padding: '8px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', border: '1px solid', transition: 'all 0.15s', background: horarioSelecionado === h ? 'var(--accent)' : 'var(--surface)', color: horarioSelecionado === h ? '#fff' : 'var(--text-primary)', borderColor: horarioSelecionado === h ? 'var(--accent)' : 'var(--border)' }}>
                              {h}
                            </button>
                          ))}
                        </div>
