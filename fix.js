...
// Atualizar layout do painel para incluir aviso dentro do content wrapper
...
// Atualizar AcessoGuard para renderizar aviso DENTRO do children wrapper
const guard = `'use client'
...
fs.writeFileSync('components/AcessoGuard.tsx', guard, 'utf8')
console.log('AcessoGuard OK!')

// Adicionar AvisoAtraso dentro do container .bdy de cada página
const paginas = [
  { file: 'app/painel/agendamentos/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/clientes/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/orcamentos/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/cobrancas/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/pagamentos/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/servicos/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/profissionais/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/relatorio/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/perfil/page.tsx', importPath: '../../components/AcessoGuard' },
  { file: 'app/painel/page.tsx', importPath: '../components/AcessoGuard' },
]

for (const { file, importPath } of paginas) {
  if (!fs.existsSync(file)) continue
  let c = fs.readFileSync(file, 'utf8')

  // Remove qualquer import ou uso anterior
  c = c.replace(/import \{ AvisoAtraso \} from '[^']+'\r?\n/g, '')
  c = c.replace(/<AvisoAtraso\/>\r?\n?/g, '')

  // Adiciona import após 'use client'
  c = c.replace("'use client'\n", `'use client'\nimport { AvisoAtraso } from '${importPath}'\n`)

  // Adiciona <AvisoAtraso/> após <div className="bdy"
  const bdyRegex = /<div className="bdy"[^>]*>/
  if (bdyRegex.test(c)) {
    c = c.replace(bdyRegex, (m) => m + '\n          <AvisoAtraso/>')
    console.log('Adicionado:', file)
  } else {
    console.log('Sem .bdy:', file)
  }
